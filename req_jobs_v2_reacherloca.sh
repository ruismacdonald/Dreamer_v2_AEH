...
#SBATCH --output=/home/ruism/projects/def-rsdjjana/ruism/Dreamer_v2_AEH/results/reacherloca_v2_state_ae/%A-%a.out
#SBATCH --error=/home/ruism/projects/def-rsdjjana/ruism/Dreamer_v2_AEH/results/reacherloca_v2_state_ae/%A-%a.err

set -e -o pipefail

# Top-level results dir on Lustre
BASE_SAVE_DIR="$HOME/projects/def-rsdjjana/ruism/Dreamer_v2_AEH/results/reacherloca_v2_state_ae"
...
RUN_DIR="${SLURM_TMPDIR}/dreamer-v2-ae-r-${SLURM_JOB_ID:-0}-${SEED}"
...
python -u "$DREAMER_SRC/dreamer.py" \
  ...
  --algo Dreamerv2 \
  --exp-name reacherloca_v2_state_ae \
  --train \
  --loca-all-phases \
  --buffer-size 2500000 \
