#!/bin/bash
#SBATCH --job-name=d_v2_aeh_r_rep_norm
...
#SBATCH --error=/home/ruism/projects/def-rsdjjana/ruism/Dreamer_v2_AEH/results/reacherloca_v2_rep_norm_state_ae/%A-%a.err

set -e -o pipefail

# Top-level results dir on Lustre
BASE_SAVE_DIR="$HOME/projects/def-rsdjjana/ruism/Dreamer_v2_AEH/results/reacherloca_v2_rep_norm_state_ae"
...
python -u "$DREAMER_SRC/dreamer.py" \
  ...
  --algo Dreamerv2 \
  --exp-name reacherloca_v2_rep_norm_state_ae \
  --train \
  --loca-all-phases \
  --buffer-size 2500000 \
